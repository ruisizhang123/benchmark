    ...
    def __init__(self, name, test, device, batch_size=None, extra_args=[]):
        ...
        if is_basic_huggingface_models(name):
            ...
            if not hasattr(self.model.config, "vocab_size"):
                self.model.config.vocab_size = self.model.language_model.get_input_embeddings().weight.size()[0]
        ...
        elif self.name in ["llama3_2_11B"]:
            self.example_inputs_image = torch.rand((self.batch_size, 1, 4, 3, 448, 448)).to(self.device)
            self.example_inputs.update({"pixel_values": self.example_inputs_image})
            self.example_inputs_ratio  = torch.randint(0, 10, (self.batch_size, 1)).to(self.device)
            self.example_inputs.update({"aspect_ratio_ids": self.example_inputs_ratio})
            self.example_inputs_ratio_mask = torch.randint(0, 2, (self.batch_size, 1, 4)).to(self.device)
            self.example_inputs.update({"aspect_ratio_mask": self.example_inputs_ratio_mask})
            self.example_inputs['labels'] = torch.randint(0, self.model.config.vocab_size, (self.batch_size, 512)).to(self.device)
        if is_training:
            self.model.train()
        else:
            self.model.eval()
